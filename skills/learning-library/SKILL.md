...
description: >-
  ...
  catalog.json（検索用の派生インデックス）を再生成して index.html の参照パスを揃える。技術書・技術トピックの
...
## コマンド: `/update [識別ワード または file名] [変更内容]`
...
| コマンド | 用途 | 必要な添付 |
|---|---|---|
| `/create [トピック]` | 新記事を `pages/<棚>/` に作りハブと catalog に追加 | 最新 index.html（＋catalog.json） |
| `/update [ワード or file] [内容]` | catalog で対象を名寄せして手直し | 最新 index.html か catalog.json（特定後に対象ファイル） |
| `/org` | `pages/` を棚別に整頓し catalog.json を再生成 | 最新 index.html＋pages/ 一式 |
